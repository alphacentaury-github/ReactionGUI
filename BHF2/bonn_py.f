! original code can be compiled only by ifort
! because of non-standard fortran
!
! modified for gfortran
!
! For python
! (1) make pyf file by using only bonn_wrap
!       bonn_f2py.f is created with bonn_wrap temporarily
!  f2py bonn_f2py.f -m bonn_py -h bonn_py.pyf
! (2) compile with f2py
!  f2py -c bonn_py.pyf bonn_gfortran.f
!
! In Windows, one have to be careful for 
!    consistency of versions of fortran and mingw 
! In Linux, there was no problem.
!  (3) may be best way 
!      python -m numpy.f2py -c -m bonn_py bonn_py.f
!      this seems to work
      subroutine bonn_wrap(xxmev,yymev,jj,vout,potid)
        ! v(1:6) for given J xxmev and yymev
        !
        ! v(1) = V_{J,J}^{S=0}(xxmev_out,yymev_in)
        ! v(2) = V_{J,J}^{S=1}(xxmev_out,yymev_in)
        ! v(3) = V_{J+1,J+1}^{S=1}(xxmev_out,yymev_in)
        ! v(4) = V_{J-1,J-1}^{S=1}(xxmev_out,yymev_in)
        ! v(5) = V_{J+1,J-1}^{S=1}(xxmev_out,yymev_in)
        ! v(6) = V_{J-1,J+1}^{S=1}(xxmev_out,yymev_in)
		!
		! potid = 0: Bonn A ps , LS 
		!       = 1: Bonn B ps , LS
		!       = 2: Bonn C ps , LS
		!       = 3: Bonn A pv , LS
		!       = 4: Bonn B pv , LS 
		!       = 5: Bonn C pv , LS 
		! Thompson equation form is not provided 
		!
		! Because code read input only once 
		! once the code is called, one cannot change potential  
		! by changing potid !! 
	  implicit real*8 (a-h,o-z)
	  real*8, intent(in) :: xxmev,yymev
      integer,intent(in) :: jj
	  real*8, intent(out),dimension(6) :: vout
	  integer,intent(in) :: potid 
	  !----common blocks for bonn
...
cf2py intent(in) xxmev
cf2py intent(in) yymev
cf2py intent(in) jj
cf2py intent(in) potid 
cf2py intent(out),dimension(6) vout
	  !--initialize bonn control
	  if (potid.eq.0) then 
	      open (unit=5,file='bonn_A_ps.inp')
	  else if (potid.eq.1) then 
	      open (unit=5,file='bonn_B_ps.inp')  
	  else if (potid.eq.2) then 
	      open (unit=5,file='bonn_C_ps.inp')   	  
	  else if (potid.eq.3) then 
	      open (unit=5,file='bonn_A_pv.inp')  	  
	  else if (potid.eq.4) then 
	      open (unit=5,file='bonn_B_pv.inp')  	  
	  else if (potid.eq.5) then 
	      open (unit=5,file='bonn_C_pv.inp')  	  
	  end if 	  
	  kread=5
	  kwrite=6
	  kpunch=7
...
c
	  ...
	  character*4 name(3)
	  character*4 nname(15)
      integer imga(3)
	  character*4 cut,cutg,end,stars !test
      !!!integer cut,cutg,end,stars
      data cut/'cut '/,cutg/'cutg'/,end/'end '/,stars/'****'/
      !integer mesong(12)
	  character*4 mesong(12) ! test
      data mesong/'0-  ','0-t ','0-st','0+  ','0+st',
     1            '1-  ','1-t ','1-tt','1-st','1-ss',
     2            '1+  ','2+  '/
      logical index
...
!      program test_bonn
!	  implicit none
!	  real*8 :: xxmev, yymev, vout(6)
!	  integer :: jj
!	  xxmev=9352.94629
!	  yymev= 9.97644
!	  jj=1
!	  call bonn_wrap(xxmev,yymev,jj,vout)
!	  write(*,*) '--input--'
!	  write(*,*) xxmev, yymev, jj
!	  write(*,*) '--output in fm unit--'
!	  write(*,*) vout*291027.7035863837 ! fm units
!	  end program test_bonn
!=======================================================
